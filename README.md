
# Birthday Notifications

   _A small library using Python to automate birthday notifications via email message._

 **Table of Contents**

1.  [About me](#Introduction1)
2.  [Technologies Used](#paragraph1)
3.  [Requirements](#paragraph2)
4.  [Installation Instructions](#paragraph3)
5.  [Usage Instructions](#paragraph4)
6.  [Documentation](#paragraph5)
7.  [Visuals](#paragraph6)
8.  [Support Information](#paragraph7)
9.  [Project Roadmap](#paragraph8)
10. [Project Status](#paragraph9)
11. [Contribution Guidelines](#paragraph10)
12. [Acknowledgments](#paragraph11)
13. [License Information](#paragraph12)
14. [Conclusion](#paragraph13)

## About me <a name="Introduction1"></a>

Hello! My name is [Melissa](https://www.linkedin.com/in/melissalehman1/). You can click on my name to view my LinkedIn profile. I am a dynamic security professional with 10 years in the industry. My background includes RAM (Remote Access Management) as a Supervisor and Salesforce Assistant Administrator as a Supervisor within the Data Entry department. I graduated with a Bachelor's in Business and am currently following my passion in IT. (Believe it or not, at the age of 12 I self taught myself to do basic code on a Macintosh!) Some of my training includes CompTIA A+ (currently studying for certification) and Introduction to QA.   

Disclosure: The above owner will be the default owner for this repo.

## Technologies Used <a name="paragraph1"></a>

1. [Python](https://www.python.org/) 
2. [SQLite3 w/ Python](https://www.geeksforgeeks.org/python-sqlite/)
3. [Selenium](https://www.selenium.dev/)
4. [Project IDX](https://developers.google.com/idx)


## Requirements <a name="paragraph2"></a>

Skill Requirements
1. Git and GitHub
2. Writing Python Code
3. Writing SQL
4. Implementing Selenium
5. Learning core QA Concepts & Methodologies

Final Project Presentation
1. Show your GitHub account
2.   Discuss 1-2 User Stories/Test Cases that you've designed (this can be a read-me file on GitHub, a google slide, or PPT presentation)
3. Reflect on what you've learned or how this training will benefit you


## Installation Instructions <a name="paragraph3"></a>

This is where you would show how to get the development environment up and running

* _Project IDX_ [here](https://developers.google.com/idx)
* _GitHub_ [here](https://github.com/)
* _Python_ [here](https://www.python.org/downloads/)
* _SQLite3_ [here](https://www.geeksforgeeks.org/python-sqlite/)

## Usage Instructions <a name="paragraph5"></a>

* _Provide instructions and examples so users/contributors can use the project with brief description of the software's capabilities

## Documentation <a name="paragraph5"></a>

This is a great place to include a hyperlink so the README file isn't bogged down. Main documentation includes:

* Installation Instructions, API Documents, Benchmark Tests, and changelog.
* [Notes](Notes.txt)

* Created database.py - received successful statement
* Created main.py - will not create family_database.db, will need to review closer for issues.

## Visuals <a name="paragraph6"></a>

![alt text](<QA Image.jpg>)
[QA Image](https://www.google.com/imgres?q=funny%20image%20for%20QA&imgurl=https%3A%2F%2Fimage.spreadshirtmedia.com%2Fimage-server%2Fv1%2Fcompositions%2FT650A2PA4005PT17X25Y17D1045511966W34899H41879%2Fviews%2F1%2Cwidth%3D550%2Cheight%3D550%2CappearanceId%3D2%2CbackgroundColor%3D000000%2CnoPt%3Dtrue%2Ffunny-qa-engineer-expert-advice-quality-assurance-mens-tall-t-shirt.jpg&imgrefurl=https%3A%2F%2Fwww.spreadshirt.com%2Fshop%2Fdesign%2Ffunny%2Bqa%2Bengineer%2Bexpert%2Badvice%2Bquality%2Bassurance%2Bmens%2Btall%2Bt-shirt-D62a30478ebe60a221642dc17%3Fsellable%3DVRlGG8jp45feyXkY5xNd-650-7&docid=7a3zb5EnqUgnrM&tbnid=YEpBZKifL22n3M&vet=12ahUKEwjfjrCJjeuJAxVEw_ACHceaI0YQM3oECFYQAA..i&w=550&h=550&hcb=2&ved=2ahUKEwjfjrCJjeuJAxVEw_ACHceaI0YQM3oECFYQAA)


## Support Information <a name="paragraph7"></a>

For Support, please reach out to your Manager [Melissa](melissalehman73@gmail.com)

## Project Roadmap <a name="paragraph8"></a>

**Current Development:**

*  **Core Functionality:**
    * Develop data file
    * Integrate with primary data source

**Long-Term Goals:**
  * Continuous research of QA and coding

* **Improvements:**
    * Optimize database performance for large datasets
    * 
* **Engagement:**
    * User feedback integration
    * Create documentation and work instructions

** *This roadmap is subject to change.* **

## Project Status <a name="paragraph9"></a>

* **Current Status:** In progress
   * [done]() <text style="color: green">Create README.md</text> </p> 
   * [done]() About me
   * [done]() Technologies Used
   * [done]() Requirements
   * [done]() Installation Instructions
   * [in progress]() <text style="color: purple">Usage Instructions</text> </p>
   * [in progress] Documentation
   * [done]() Visuals
   * [done]() Support Information
   * [done]() Project Roadmap
   * [in progress] Project Status
   * [done]() Contribution Guidelines
   * [done]() Acknowledgments
   * [done]() License Information
   * [done]() Conclusion


## Contribution Guidelines <a name="paragraph10"></a>

[Must Read Code of Conduct](Licenses_and_Code/CODE_OF_CONDUCT.md)

## Acknowledgments <a name="paragraph11"></a>

Introduction to Quality Assurance [Valerie Lockhart](https://www.linkedin.com/in/valockhart/)

Introduction to Quality Assurance [Sage Faraday]( https://www.linkedin.com/in/sage-faraday/)

How to do Markdown Anchor Links for Table of Contents (GitHub) [Patrick Jackson](https://www.google.com/search?q=how+to+create+a+hyperlink+to+another+section+in+readme+file&sca_esv=77c72257878c30ef&sxsrf=ADLYWIJH4OmEzTwLumF99wxM5vb3g71lSA%3A1732036606512&ei=_sc8Z63xHu6mptQPmLmPiAw&ved=0ahUKEwit6KyV8-iJAxVuk4kEHZjcA8EQ4dUDCA8&uact=5&oq=how+to+create+a+hyperlink+to+another+section+in+readme+file&gs_lp=&sclient=gws-wiz-serp#fpstate=ive&vld=cid:da7934f5,vid:EKqhENATIKg,st:52)

15 Elements to Include in Your README Document [Davor](https://www.archbee.com/blog/readme-document-elements)

How to Write a Good README File for Your Github Project [Hillary Nyakundi](https://www.freecodecamp.org/news/how-to-write-a-good-readme-file/)



## License Information <a name="paragraph12"></a>

This project is licensed under the terms of the MIT license.

 [MIT License](License.txt)

## Conclusion <a name="paragraph13"></a>

Going into the Intro to QA course, I had a limited understanding of the field.  The course has opened my eyes to the diverse aspects of QA, from test planning and execution to automation and quality assurance methodologies.  One of my biggest takeaways was learning how to write effective test cases.  While challenging at first, practicing this skill has helped me think critically about software functionality and identify potential issues. I'm excited to apply this knowledge in my upcoming projects.  


**End of Project**
=

